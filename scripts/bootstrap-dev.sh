...
# Idempotent Terraform backend bootstrap for DEV.
...
# - writes environments/dev/backend.tfbackend
# - optionally runs terraform init

ENVIRONMENT="dev"
LOCATION="${LOCATION:-eastus}"
TF_DIR="${TF_DIR:-environments/dev}"
TFSTATE_RG="${TFSTATE_RG:-tfstate-dev-rg}"
...
# Derive a stable globally unique-ish storage account name if user did not provide one.
if [[ -z "$TFSTATE_SA" ]]; then
  SUB_SHORT="$(echo "$SUBSCRIPTION_ID" | tr -d '-' | cut -c1-10 | tr '[:upper:]' '[:lower:]')"
  TFSTATE_SA="tfstate${ENVIRONMENT}${SUB_SHORT}"
fi

# Storage account name rules: 3-24 lowercase alnum.
...
if ! az storage account show --name "$TFSTATE_SA" --resource-group "$TFSTATE_RG" >/dev/null 2>&1; then
  ...
  # Normalize key backend settings on reuse.
...
# Optional AAD role for the Terraform service principal.
if [[ -n "$SP_APP_ID" ]]; then
